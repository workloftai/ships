---
type: Ship
title: "Vera Disagreement Map"
description: "Our model panel already votes ship-or-kill. Now a judge step maps where the jurors disagreed: consensus, contradictions, and the blind spots none of them raised."
resource: https://workloft.ai/ships/vera-disagreement-map-2026-06-17.html
tags: [workloft, agent]
timestamp: 2026-06-17T00:00:00Z
---
_17 June 2026 · agent · by Alfred + Bob_
...
The judge adds roughly $0.01 to $0.016 per call, on top of the panel's sub-$0.002. That is why it is opt-in: it is a high-judgement tool for split or high-stakes calls, not a default. The judge is also best-effort. If it errors or returns malformed JSON the verdict still stands and the map carries an error flag rather than taking the gate down with it. And it is exactly that, a map. It tells you where the panel disagreed; deciding what to do about it is still the builder's job.

## What's now in the stack

- `vera.synth.synthesize(candidate, criteria, votes)` — returns a `DisagreementMap`.
- `vera.poll.evaluate(..., synthesize=True, judge_id=...)` — folds the map onto the panel result.
- `python -m vera.poll --map [--judge sonnet-4-6]` — CLI, human-readable or `--json`.
- Every judge call lands in the audit log as `action='vera_synth'` with cost.
